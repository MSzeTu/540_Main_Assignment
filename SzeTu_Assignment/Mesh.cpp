...
Mesh::Mesh(Vertex* verticesParam, int vertexNum, unsigned int* usIndices, int numIndices, 
	Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	CreateBuffers(verticesParam, vertexNum, usIndices, numIndices, device);
}
...
Mesh::~Mesh() //Destructor (Currently Empty)
{
}

Microsoft::WRL::ComPtr<ID3D11Buffer> Mesh::GetVertexBuffer() //returns the Vertex Buffer
{
	return vertexBuffer;
}

Microsoft::WRL::ComPtr<ID3D11Buffer> Mesh::GetIndexBuffer() //returns the Index Buffer
{
	return indexBuffer;
}
...
void Mesh::Draw(Microsoft::WRL::ComPtr<ID3D11DeviceContext> context)
{
	...
	//The actual drawing
	context->DrawIndexed(
		GetIndexCount(), //Number of Indices
		0, //Offset of first index
		0); //Offset added to each index
}

void Mesh::CreateBuffers(Vertex* verticesParam, int vertexNum, unsigned int* usIndices, int numIndices, 
	Microsoft::WRL::ComPtr<ID3D11Device> device)
{	
	//Calculate tangents of each vertex
	CalculateTangents(verticesParam, vertexNum, usIndices, numIndices);
	...
	//Set up vertices
	Vertex* vertices = verticesParam;

	//Set up indices
	unsigned int* indicesArr = usIndices;

	//Create Vertex Buffer Description
	D3D11_BUFFER_DESC vbd = {};
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(Vertex) * vertexNum; //Multiply by number of vertices
	vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER; // Tells DirectX this is a vertex buffer
	vbd.CPUAccessFlags = 0;
	vbd.MiscFlags = 0;
	vbd.StructureByteStride = 0;

	//Create struct for vertex data
	...
	ibd.ByteWidth = sizeof(unsigned int) * indexCount;	//Multiply by number of indices
	...
	initialIndexData.pSysMem = indicesArr;
...
//   code of your own design. CONSIDER IT CITED RIGHT HERE 
//
