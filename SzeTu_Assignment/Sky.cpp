...
Sky::Sky(std::shared_ptr<Mesh> paramM, Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> cubeMap,
    Microsoft::WRL::ComPtr<ID3D11SamplerState> paramSamplerOptions, Microsoft::WRL::ComPtr<ID3D11Device> device,
    Microsoft::WRL::ComPtr<ID3D11DeviceContext> context, std::shared_ptr<SimpleVertexShader> skyVS, std::shared_ptr<SimplePixelShader> skyPS)
    :
    skySRV(cubeMap),
    meshPtr(paramM),
    device(device),
    context(context),
    samplerOptions(paramSamplerOptions),
    skyVS(skyVS),
    skyPS(skyPS)
{
	...
	Microsoft::WRL::ComPtr<ID3D11DeviceContext> contextParam)
{
	...
	skyVS = skyVSParam;
	skyPS = skyPSParam;
	samplerOptions = samplerOptionsParam;
	device = deviceParam;
...
Sky::~Sky() //Using smart pointers, so nothing to clean up
{
}
...
//   code of your own design. THIS IS NOT MY OWN CODE, CONSIDER IT CITED
//
...
Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> Sky::CreateCubemap(const wchar_t* right, const wchar_t* left, const wchar_t* up, const wchar_t* down, const wchar_t* front, const wchar_t* back)
{
	...
	DirectX::CreateWICTextureFromFile(device.Get(), right, (ID3D11Resource**)&textures[0], 0);
	DirectX::CreateWICTextureFromFile(device.Get(), left, (ID3D11Resource**)&textures[1], 0);
	DirectX::CreateWICTextureFromFile(device.Get(), up, (ID3D11Resource**)&textures[2], 0);
	DirectX::CreateWICTextureFromFile(device.Get(), down, (ID3D11Resource**)&textures[3], 0);
	DirectX::CreateWICTextureFromFile(device.Get(), front, (ID3D11Resource**)&textures[4], 0);
	DirectX::CreateWICTextureFromFile(device.Get(), back, (ID3D11Resource**)&textures[5], 0);
